...
<template>
  <BTableSimple hover small caption-top responsive>
    <caption
      class="font-extrabold leading-[.2px] tracking-[1px] text-[14px] text-[#1d212f] dark:text-slate-400 uppercase"
    >
    ...
    <BTbody>
      <BTr v-for="crypto in allCoins" :key="crypto.Name">
        ...
        <BTd class="py-3 px-4">
          <span
            :class="[
              crypto.up ? 'text-green-500' : 'text-red-500',
              crypto.orange && 'text-orange-500',
            ]"
          >
            {{ crypto.Change_24H }}
          </span>
        ...
        <BTd class="py-3 px-4">
          <button
            class="border-[1px] border-violet-600 px-2 py-[2px] text-violet-600 rounded-[0.2rem]"
          >
            Trade
          </button>
        </BTd>
